...
// Get squad's pending invitations (join requests)
const getSquadInvitations = async (req, res) => {
  try {
    ...
    const invitations = await Invitation.find(query)
      .populate('invitedUserId', 'name pubgId email')
      .sort({ createdAt: -1 });

    res.json({ success: true, data: invitations });

  } catch (error) {
    console.error('Error in getSquadInvitations:', error);
    res.status(500).json({ success: false, error: 'Server error' });
  }
};
