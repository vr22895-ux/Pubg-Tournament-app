...
// Send Room ID and Password to registered players
exports.sendRoomDetails = async (req, res) => {
  try {
    ...
    // 1. Update Match with Room Details
    const match = await Match.findByIdAndUpdate(
      matchId,
      { roomId, roomPassword },
      { new: true }
    ...
    // 2. Notify all registered players
    const registeredUserIds = match.registeredPlayers
    ...
    const notificationPromises = registeredUserIds.map(userId =>
      notificationController.sendNotification({
        userId,
        title: `Room Details: ${match.name}`,
        message: `Room ID: ${roomId}, Password: ${roomPassword}`,
        type: 'match_start',
        data: { matchId, roomId, roomPassword }
      })
    );
    ...
    res.json({
      success: true,
      message: `Room details sent to ${registeredUserIds.length} players`,
